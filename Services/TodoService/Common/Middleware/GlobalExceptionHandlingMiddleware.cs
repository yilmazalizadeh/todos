...
namespace TodoService.Common.Middleware;
    ...
    ILogger<GlobalExceptionHandlingMiddleware> logger)
{
    ...
    private async Task HandleExceptionAsync(HttpContext context, Exception exception)
    {
        ...
        if (exception is TodoServiceException todoServiceException)
        {
            statusCode = todoServiceException switch
            {
                NotFoundException => StatusCodes.Status404NotFound,
                ValidationException => StatusCodes.Status400BadRequest,
                _ => StatusCodes.Status500InternalServerError
            };
            error = todoServiceException.Message;
            type = todoServiceException.GetType().Name;
        }
        else
        {
